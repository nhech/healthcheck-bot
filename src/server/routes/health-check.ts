...
export default (app: Express, makeRequest: MakeRequest) => {
    app.get('/health-check', (req: any, res) => {
        ...
        makeRequest(req.query.url)
            .then(() => res.status(httpStatus.OK).json({ status: RESPONSE_STATUS.ok }))
            .catch(() => res.status(httpStatus.BAD_REQUEST).json({ status: RESPONSE_STATUS.error }));
    });
